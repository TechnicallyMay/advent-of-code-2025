...
* Generate a graph of connections. Each splitter has 2 connections
```txt
...^... <- Left connection is to the end, right connection is to the next splitter
....^..
```
* Data structure
```json
{
    leftConnection: nil,
    rightConnection: refToNextNode
}
* Then, we just have a binary tree problem to solve
```

## Plan
* Parse file into nodes by index
* Loop back over to populate connections
* Recursively solve for # paths
## Full Text
--- Part Two ---
