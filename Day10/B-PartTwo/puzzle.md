...
## Summary
* Same as before, instead of light configuration (w/ toggle) each button increments a joltage meter

## Plan
* Same as before. Dump binary, add arrays
```
    [0,0,0,0] <- Start
ADD [1,0,0,1] <- Button (0,3)
    [1,0,0,1]
ADD [1,1,1,1] <- Button (0,1,2,3)
    [2,1,1,2]
```
